const ExpenseHead = require("../models/expenseHeadModel.js");
const School = require("../models/schoolModel.js");
const catchAsyncErrors = require("../middlewares/catchAsyncErrors.js");
const ErrorHandler = require("../utils/errorHandler.js");

//TODO: CREATE Expense Head
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new ExpenseHead(dataToSave);

    const isDuplication = await ExpenseHead.findOne({ school: School_Id, name: req.body.name }).exec();
    if (isDuplication) {
...
//TODO: GET ALL Expense Head FOR A SCHOOL
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  //! HEADERS
  const School_Id = req.headers["x-school-id"];
  //! HEADERS

  const data = await ExpenseHead.find({ school: School_Id }).populate("school").exec();
...
//TODO: GET SINGLE Expense Head detail
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;

  await ExpenseHead.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await ExpenseHead.findById(id).exec();
...
//TODO: DELETE A Expense Head
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await ExpenseHead.findById(id).exec();
...
// TODO: GET ALL Expense Head
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await ExpenseHead.find({}).populate("school").exec();
