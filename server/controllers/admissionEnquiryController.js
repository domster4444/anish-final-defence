const AdmissionEnquiry = require("../models/admissionEnquiryModel");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new AdmissionEnquiry(dataToSave);
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await AdmissionEnquiry.find({ school: School_Id }).populate("school").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await AdmissionEnquiry.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await AdmissionEnquiry.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await AdmissionEnquiry.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await AdmissionEnquiry.find({}).populate("school").exec();
