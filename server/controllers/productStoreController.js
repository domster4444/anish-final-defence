const ProductStore = require("../models/productStoreModel.js");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new ProductStore(dataToSave);

    const isDuplication = await ProductStore.findOne({ school: School_Id, name: req.body.name }).exec();
    if (isDuplication) {
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await ProductStore.find({ school: School_Id }).populate("school").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await ProductStore.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await ProductStore.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await ProductStore.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await ProductStore.find({}).populate("school").exec();
