const ReviewProductModel = require("../models/reviewProductModel");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new ReviewProductModel(newRecordData);
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  console.log("HIT HIS T IHS");
  //! HEADERS
  const School_Id = req.headers["x-school-id"];
  //! HEADERS
  const record = await ReviewProductModel.find({ school: School_Id }).populate("school").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await ReviewProductModel.findById(id)
    .populate("school")
    .exec((error, record) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const existingRecord = await ReviewProductModel.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  const record = await ReviewProductModel.find({}).populate("school").exec();
