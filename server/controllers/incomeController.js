...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new Income(dataToSave);
...
//TODO: GET ALL Income FOR A SCHOOL
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await Income.find({ school: School_Id }).populate("school").exec();
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await Income.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  const { id } = req.params;
  try {
    const recordToDelete = await Income.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  //! HEADERS
  try {
    const allRecords = await Income.find({}).populate("school").exec();
