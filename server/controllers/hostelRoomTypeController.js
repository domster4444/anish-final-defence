const HostelRoomType = require("../models/hostelRoomTypeModel.js");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newHostelRoom = new HostelRoomType(dataToSave);

    const isDuplication = await HostelRoomType.findOne({ school: School_Id, name: req.body.name }).exec();
    if (isDuplication) {
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await HostelRoomType.find({ school: School_Id }).populate("school").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await HostelRoomType.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await HostelRoomType.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const HostelRoom = await HostelRoomType.findById(id).exec();

    if (!HostelRoom) {
      return next(new ErrorHandler("record not found", 404));
    }

    await HostelRoom.remove();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const HostelRoom = await HostelRoomType.find({}).populate("school").exec();
