const PostalReceive = require("../models/postalReceiveModel.js");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new PostalReceive(newRecordData);
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const School_Id = req.headers["x-school-id"];
  //! HEADERS
  const record = await PostalReceive.find({ school: School_Id }).populate("school").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await PostalReceive.findById(id)
    .populate("school")
    .exec((error, record) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const existingRecord = await PostalReceive.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  const record = await PostalReceive.find({}).populate("school").exec();
