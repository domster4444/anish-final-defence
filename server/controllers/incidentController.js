...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new Incident(dataToSave);
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await Incident.find({ school: School_Id }).populate("school").exec();
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await Incident.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await Incident.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await Incident.find({}).populate("school").exec();
