const ExamTerm = require("../models/examTermModel.js");
const School = require("../models/schoolModel.js");
const catchAsyncErrors = require("../middlewares/catchAsyncErrors.js");
const ErrorHandler = require("../utils/errorHandler.js");

//TODO: CREATE Exam Term
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new ExamTerm(dataToSave);

    const isDuplication = await ExamTerm.findOne({ school: School_Id, name: req.body.name }).exec();
    if (isDuplication) {
...
//TODO: GET ALL Exam Term FOR A SCHOOL
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  //! HEADERS
  const School_Id = req.headers["x-school-id"];
  //! HEADERS

  const data = await ExamTerm.find({ school: School_Id }).populate("school").exec();
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await ExamTerm.findById(id).exec();
...
//TODO: DELETE A Exam Term
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await ExamTerm.findById(id).exec();
...
// TODO: GET ALL Exam Term
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await ExamTerm.find({}).populate("school").exec();
