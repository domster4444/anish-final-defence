const ProductSupplier = require("../models/productSupplierModel.js");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new ProductSupplier(dataToSave);

    const isDuplication = await ProductSupplier.findOne({ school: School_Id, name: req.body.name }).exec();
    if (isDuplication) {
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await ProductSupplier.find({ school: School_Id }).populate("school").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await ProductSupplier.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await ProductSupplier.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await ProductSupplier.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await ProductSupplier.find({}).populate("school").exec();
