const LiveEmployeeMeeting = require("../models/liveEmployeeMeetingModel");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  console.log("i hvae mbeen hit ***");
  console.log(req.body);
  //! HEADERS
  const School_Id = req.headers["x-school-id"];
  //! HEADERS
  req.body.staffs = JSON.parse(req.body.staffs);
  ...
  try {
    ...
    const newRecord = new LiveEmployeeMeeting(dataToSave);
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await LiveEmployeeMeeting.find({ school: School_Id }).populate("school").populate("createdBy").populate("staffs").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await LiveEmployeeMeeting.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await LiveEmployeeMeeting.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await LiveEmployeeMeeting.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await LiveEmployeeMeeting.find({}).populate("school").exec();
