const LiveClassMeeting = require("../models/liveClassMeetingModel.js");
...
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new LiveClassMeeting(dataToSave);
...
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  ...
  const data = await LiveClassMeeting.find({ school: School_Id }).populate("school").populate("createdBy").populate("staffs").exec();
...
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await LiveClassMeeting.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await LiveClassMeeting.findById(id).exec();
...
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  try {
    const recordToDelete = await LiveClassMeeting.findById(id).exec();
...
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  try {
    const allRecords = await LiveClassMeeting.find({}).populate("school").exec();
