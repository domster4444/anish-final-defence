...
const AchievementCertificateDesign = require("../models/achievementCertificateDesignModel");
...
exports.addRecordController = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newDesignRecord = new AchievementCertificateDesign({
      school: School_Id,
...
exports.getAllRecordForASchoolController = catchAsyncErrors(async (req, res, next) => {
  const School_Id = req.headers["x-school-id"];
  const allRecord = await AchievementCertificateDesign.find({ school: School_Id }).populate("school").exec();
  if (!allRecord) {
...
exports.deleteRecordController = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const record = await AchievementCertificateDesign.findById(recordId);
    if (!record) {
      return res.status(404).json({
        success: false,
