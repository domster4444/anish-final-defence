const StudentOneTimeAttendance = require("../models/studentOneTimeAttendanceModel");
const School = require("../models/schoolModel.js");
const catchAsyncErrors = require("../middlewares/catchAsyncErrors.js");
const ErrorHandler = require("../utils/errorHandler.js");
const { addFile, deleteFile } = require("../utils/fileManipulation.js");

//TODO: CREATE StudentOneTimeAttendance
exports.createRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const newRecord = new StudentOneTimeAttendance(dataToSave);

    const isDuplication = await StudentOneTimeAttendance.findOne({ school: School_Id, attendanceId: req.body.attendanceId }).exec();
    if (isDuplication) {
...
//TODO: GET ALL StudentOneTimeAttendance FOR A SCHOOL
exports.getAllRecordForSchool = catchAsyncErrors(async (req, res, next) => {
  //! HEADERS
  const School_Id = req.headers["x-school-id"];
  const School_Folder_Name = req.headers["x-school-unique-id"];
  console.log("hello", School_Id)
  //! HEADERS
  let data = await StudentOneTimeAttendance.find({ school: School_Id }).populate("school").populate("student").exec();
  data = data.map((item) => {
    return {
      id: item._id,
...
//TODO: GET SINGLE StudentOneTimeAttendance  detail
exports.getSingleRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  await StudentOneTimeAttendance.findById(id)
    .populate("school")
    .exec((error, data) => {
      if (error) {
...
//TODO: UPDATE StudentOneTimeAttendance
exports.updateRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const ExistingRecord = await StudentOneTimeAttendance.findById(id).exec();
...
//TODO: DELETE A StudentOneTimeAttendance
exports.deleteRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  const { id } = req.params;
  try {
    const recordToDelete = await StudentOneTimeAttendance.findById(id).exec();
...
// TODO: GET ALL StudentOneTimeAttendance
exports.getAllRecord = catchAsyncErrors(async (req, res, next) => {
  ...
  //! HEADERS
  try {
    const allRecords = await StudentOneTimeAttendance.find({}).populate("school").exec();
