...
import { usePatchUpdateRecordMutation } from "app/GlobalRedux/API/feeTypeApi";
...
const EditBookForm = ({ setInitialFormData, initialFormData, fetchRecordAndSetTable, closeEditModalHandler }) => {
  ...
  return (
    <>
      ...
      <input
        name='feeCode'
        value={initialFormData?.feeCode}
        onChange={(e) => {
          handleInputChange(e);
          setInitialFormData({ ...initialFormData, feeCode: e.target.value });
        }}
        type='text'
        className='w-full p-3 mb-3 rounded-md'
        placeholder='Enter fee group name'
