...
import { usePostAddRecordMutation } from "app/GlobalRedux/API/feeTypeApi";
...
const AddForm = ({ fetchRecordAndSetTable, closeAddModalHandler }) => {
  ...
  return (
    <>
      ...
      <input autoComplete='off' name='name' onChange={handleInputChange} type='text' className='w-full p-3 mb-3 rounded-md' placeholder='Enter fee type name' />

      <label className='block text-sm font-medium text-gray-700 mt-4'>
        Fee Code <span className='text-red-500'>*</span>
      </label>
      <input autoComplete='off' name='feeCode' onChange={handleInputChange} type='text' className='w-full p-3 mb-3 rounded-md' placeholder='Enter fee code' />
