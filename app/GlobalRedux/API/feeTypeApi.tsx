...
export const feeTypeApi = createApi({
  reducerPath: "feeTypeApi",
  ...
  endpoints: (builder) => ({
    ...
    getAllRecordForSpecificSchool: builder.mutation({
      query: () => {
        return {
          url: `feeType/get-all-for-school`,
          method: "POST",
          formData: true,
          headers: {
            "x-school-id": getDataByValue("schoolId"),
    ...
    patchUpdateRecord: builder.mutation({
      query: (payload) => {
        console.log(payload);
        return {
          url: `feeType/update/${payload._id}`,
          method: "PATCH",
...
export const { usePostAddRecordMutation, useGetAllRecordForSpecificSchoolMutation, useDeleteRecordMutation, usePatchUpdateRecordMutation } = feeTypeApi;
