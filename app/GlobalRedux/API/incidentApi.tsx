...
export const incidentApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getAllRecordForSpecificSchool: builder.mutation({
      query: () => {
        return {
          url: `incident/get-all-for-school`,
          method: "POST",
          formData: true,
          headers: {
            "x-school-id": getDataByValue("schoolId"),
    ...
    patchUpdateRecord: builder.mutation({
      query: (payload) => {
        console.log(payload);
        return {
          url: `incident/update/${payload._id}`,
          method: "PATCH",
...
export const { usePostAddRecordMutation, useGetAllRecordForSpecificSchoolMutation, useDeleteRecordMutation, usePatchUpdateRecordMutation } = incidentApi;
