...
export const visitorBookApi = createApi({
  reducerPath: "visitorBookApi",
  ...
  endpoints: (builder) => ({
    ...
    getAllRecordForSpecificSchool: builder.mutation({
      query: () => {
        return {
          url: `visitorBook/get-all-for-school`,
          method: "POST",
          formData: true,
          headers: {
            "x-school-id": getDataByValue("schoolId"),
    ...
    patchUpdateRecord: builder.mutation({
      query: (payload) => {
        console.log(payload);
        return {
          url: `visitorBook/update/${payload._id}`,
          method: "PATCH",
...
export const { usePostAddRecordMutation, useGetAllRecordForSpecificSchoolMutation, useDeleteRecordMutation, usePatchUpdateRecordMutation } = visitorBookApi;
